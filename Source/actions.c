...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/10 17:23:11 by pedromota         #+#    #+#             */
/*   Updated: 2023/12/10 17:35:03 by pedromota        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	messages(char *str, t_philo *philo)
{
	...
	if (ft_strcmp(DIED, str) == 0 && philo->database->b_dead == 0)
	{
		printf("%lu %u %s\n", time, philo->_id, str);
		philo->database->b_dead = 1;
	}
	if (!philo->database->b_dead)
		printf("%lu %u %s\n", time, philo->_id, str);
